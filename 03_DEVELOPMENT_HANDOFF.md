...
### 4.11 장터 4구역과 튜토리얼 위치 버그 수정 (2026-08-06)

**장터는 2×2 구역이다.** 한 줄에 늘어놓던 방식을 버렸다.
...
- **화공은 구역에서 뺐다.** 1월을 2월로 바꾸는 카드가 화면 4분의 1을 차지할 값어치가 없다. 콘텐츠는 살려 두려고 `화공 묶음` 꾸러미 안에만 남겼다.
- 구역 정의는 `MarketScreen`의 `DEPARTMENTS`, 수량은 `game.ts`의 `SHOP_DEPARTMENT_SIZES`에 있다. 둘 다 고쳐야 구역이 바뀐다.
- 꾸러미를 열면 4구역 그리드가 후보 랙 하나로 바뀐다. 화면 이동은 없다.

**부적 6종 추가.** 노름꾼의 배짱(냥 3마다 배수 +1), 빈 사당(빈 칸마다 배수 +3), 열두 달의 눈(득점한 월 종류마다 ×1.15), 외날(한 장으로 득점 시 월 합 +20·배수 +6), 까치 메아리(새 그림패 재발동), 광 곳간(광마다 월 합 +12). 전부 기존 effectKey를 쓰므로 엔진 변경은 없다.

**튜토리얼 말풍선이 화면 밖으로 나가던 버그.** 수집판처럼 오른쪽 끝에 붙은 대상은 말풍선이 뷰포트를 넘어가 다음 버튼을 누를 수 없었다.

- `placeCallout()`이 아래 → 위 → 왼쪽 → 오른쪽 순으로 공간을 재고, 마지막에 양축을 뷰포트 안으로 clamp한다. 말풍선의 실제 크기를 `getBoundingClientRect()`로 재서 계산한다.
- 측정과 배치를 같은 `requestAnimationFrame` 루프에서 처리한다. 두 개의 상태로 나누면 서로 어긋나고, `useEffect` 안에서 `setState`를 부르면 렌더가 연쇄된다.

**장터 튜토리얼이 붙었다.** 전체 19단계가 됐다. 냥 → 부적전 → 비결서점 → 꾸러미방 → 금단장 → 첫 부적 구매 → 리롤 → 다음 판.

### 4.12 덱 손질방과 카드 효과 태그 (2026-08-06)

**꾸러미방을 덱 손질방으로 바꿨다.** 무료로 물건을 주는 구조를 없애고, 덱 자체를 바꾸는 구역으로 만들었다.

| 구역 | 파는 것 | 수량 |
|---|---|---|
| 부적전 | 지속 발동 부적 | 2 |
| 비결서점 | 족보 배수 영구 성장 (무작위) | 2 |
| 덱 손질방 | 카드 묶음 2 + 소각 1 | 3 |
| 금단장 | 강한 효과 + 영구 대가 | 1 |

- 카드 묶음은 두 종류다. `pack_hwatu_small`(4냥, 후보 3장 중 1장), `pack_hwatu_large`(7냥, 후보 5장 중 2장).
- 사면 `pendingPack`이 서고 `PackPickModal`이 열린다. 고른 카드는 덱에 영구히 들어간다. `PICK_PACK_CARD`로 한 장씩 집고, 남은 픽이 0이면 자동으로 닫힌다.
- 소각은 기존 화공 `p_burn`을 그대로 쓴다. 덱 손질방은 `WORKSHOP_PAINTER_IDS`에 적힌 화공만 취급하므로, 월을 바꾸는 자잘한 화공은 장터에 뜨지 않는다.
- 이전 팩 정의 5종(부적·화공·비결·금단 묶음)은 제거했다. `PACKS`는 이제 카드 묶음 2종뿐이다.

**카드 효과 태그 — 라벨만 있고 동작은 없다.**

`game/content/card-effects.ts`에 8종을 정의했다. 곳간패(끝까지 들고 있으면 3냥), 짝패(함께 제출하면 배수 +2), 무거운 달, 메아리패, 행운패, 수집패, 금칠패, 고집패.

- **점수 엔진은 이 태그를 아직 읽지 않는다.** 태그가 붙은 카드도 평범한 카드와 똑같이 동작한다. 문구를 먼저 플레이테스트하려고 라벨만 붙여 둔 상태다.
- 카드 묶음에서 나오는 카드마다 `rollCardEffectTag()`로 하나씩 굴려 `card.effectTagId`에 넣는다. 가중치가 있어 강한 태그는 드물게 나온다.
- 나중에 동작을 붙일 때는 `card.effectTagId`를 읽어 해당 엔진 모듈에서 분기한다. **`card.tags`를 재사용하지 말 것** — 그 배열은 이미 종류·족보 판정(`bird`, `rain`, `cup`)을 돌리고 있어서 이름이 겹치면 점수 버그가 된다.
- 화면에서는 카드 우상단 보라색 `효` 배지와 호버 카드에 표시된다.

**튜토리얼 말풍선이 화면 밖으로 나가던 버그.** `placeCallout()`이 아래 → 위 → 왼쪽 → 오른쪽 순으로 공간을 재고 양축을 뷰포트 안으로 clamp한다. 수집판처럼 오른쪽 끝에 붙은 세로로 긴 대상이 문제였다.

### 4.13 고 문턱 재설계 · 손패 정렬 · 튜토리얼 순서 (2026-08-06)

**고가 너무 쉬웠던 진짜 이유는 비율이 아니라 기준이었다.** 문턱이 목표 점수만 보고 있어서, 목표를 크게 넘겨 클리어하면 고를 외치는 순간 이미 문턱을 넘어 있었다. 한 손으로 2배를 내면 1고가 공짜였다.

```
목표 840 / 현재 1680 → (구) 1고 문턱 1260 → 추가로 필요 0점
목표 840 / 현재 1680 → (신) 1고 문턱 2520 → 추가로 필요 840점
```

- 문턱 = `max(목표 × 비율, 선언 시점 점수 × 1.5)`. 넘긴 만큼 문턱도 따라 올라가 항상 실질적인 추가 점수를 요구한다.
- 비율도 함께 올렸다. 1고 1.5 → **1.8**, 2고 2.2 → **2.8**, 3고 3.2 → **4.2**.
- 위험이 커진 만큼 판돈도 올렸다. 1고 1.5 → **1.7**, 2고 2.25 → **2.7**, 3고 3.4 → **4.2**.
- **문턱은 선언 시점에 `chain.goRequirement`로 박제한다.** 매번 다시 계산하면 플레이어가 점수를 낼 때마다 문턱이 같이 올라가 영원히 못 넘는다. `getRoundRequirement()`는 이 값을 읽고, `getNextGoRequirement()`가 "지금 고를 걸면 얼마"를 미리 보여 준다.

**손패 정렬.** `handSort`가 `"month" | "kind"`이고 손패 위 스위치로 바꾼다. 뽑을 때마다(`refillHand`, `startStage`) 다시 정렬하므로 순서가 흐트러지지 않는다. 같은 값이면 광 → 동물 → 띠 → 피 순으로 갈린다.

**튜토리얼 고/스톱 순서 버그.** 고를 선언한 직후 아직 새 문턱을 못 넘겼는데 "스톱을 누르세요"가 떴다. 스톱 단계의 `when`이 `goCount >= 1`만 봐서 플레이 화면에서도 참이었다.

- 스톱 단계는 `screen === "decision" && goCount >= 1`일 때만 뜬다.
- 그 사이를 메우는 `go-chase` 단계를 넣었다. 고 진행 띠를 비추고 문턱까지 계속 제출하라고 안내한다.
- 전체 20단계가 됐다.

### 4.14 호버 카드 상시 노출 버그 · 종류 변형 태그 (2026-08-06)

**손패 호버 카드가 항상 떠 있던 버그.** 파이썬 `str.replace`가 모든 일치를 바꾼다는 걸 놓쳐서, 효과 배지 CSS를 삽입할 때 hover 선택자 한가운데가 잘렸다.

```css
/* 잘린 결과 */
.hwatu-card:hover .hwatu-card__hint,
.hwatu-card:focus-visible /* 주석이 선택자를 끊어 버림 */
.hwatu-card__effect { ... }
.hwatu-card__hint { opacity: 1; }   /* ← 조건 없이 항상 보임 */
```

`.hwatu-card__hint {`가 `.hwatu-card:focus-visible .hwatu-card__hint {`의 부분 문자열이라 두 곳이 함께 치환됐다. **CSS를 스크립트로 고칠 때 선택자 조각을 `replace`의 키로 쓰면 안 된다.** 부분 문자열 충돌이 조용히 규칙을 쪼갠다.

**종류를 바꾸는 효과 태그를 추가했다.** 전체 12종이 됐다.

| 태그 | 내용 |
|---|---|
| 광/동물/띠 취급 | 원래 종류에 더해 그 종류로도 셈 |
| 덧피 | 피값 +1 — 피는 쌍피, 쌍피는 삼쌍피 |
| 쌍패 | 제 종류로 두 장 취급 — 광이면 쌍광 |

여전히 **라벨만이고 엔진은 읽지 않는다.** 다만 나중에 붙일 때 어디를 고쳐야 하는지 `card-effects.ts` 주석에 적어 뒀다. 특히 "광 취급" 계열은 `getEffectiveCardRole`·`matchesKind`·`hasKind`가 **단일 종류가 아니라 집합을 반환하도록** 바뀌어야 한다. 그 리팩터링이 이 계열의 실제 비용이다.

### 4.15 고집패 구현 · 흔들기/폭탄 선택 · 좌우 레일 교체 (2026-08-06)

**고집패가 실제로 안 버려진다.** 효과 태그는 라벨만이라 "버릴 수 없다"고 적혀 있어도 다른 카드와 묶으면 함께 버려졌다. 플레이어가 가장 먼저 시험해 보는 문구라서 이 하나만 엔진에 붙였다.

- `isUndiscardable(card)`가 `effectTagId === "stubborn"`을 본다. 나머지 11종은 여전히 라벨이다.
- 섞어서 버리면 고집패만 손에 남고 나머지는 버려진다. 고집패만 골라 버리면 버리기 횟수도 소모되지 않는다.

**흔들기는 제출 시점의 선택이 됐다.** 제출 전에 누르던 토글을 없앴다.

- 같은 월 세 장을 제출하면 `pendingShakeChoice`가 서고 모달이 열린다. **점수는 아직 계산되지 않는다** — 폭탄이 이번 손의 월 합을 바꾸기 때문에 선택이 채점보다 먼저여야 한다.
- `RESOLVE_SHAKE`가 선택을 넣고 곧바로 `submitHand`를 다시 부른다. 두 단계 제출이다.
- 흔들기는 판돈 +20%, 폭탄은 월 합 +24. 마당(yard)이 꺼져 있어 폭탄이 영영 안 나오던 문제도 같이 풀렸다.

**수집판에 줄마다 이미지 슬롯을 넣었다.** `.collection-board__icon[data-asset-tag]`이며 정사각이다.

**좌우 레일을 맞바꿨다.** 수집판이 왼쪽, 점수판이 오른쪽이다. 그리드 컬럼 폭도 함께 바꿔서 각자 원래 크기를 유지한다.
